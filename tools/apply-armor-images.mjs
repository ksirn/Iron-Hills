/**
 * Iron Hills — автоприменение картинок брони и щитов.
 *
 * Сканирует `icons/items/armor/{id}.{ext}`, проставляет в ARMORS поле
 * `img: "systems/iron-hills-system/icons/items/armor/{file}"`.
 *
 *   node tools/apply-armor-images.mjs
 *   node tools/apply-armor-images.mjs --dry-run
 */
...
const ICONS_DIR = resolve(ROOT, "icons/items/armor");
...
function applyImagesToCatalog(images) {
  ...
  for (const [id, { fileName }] of images) {
    const imgPath = `systems/iron-hills-system/icons/items/armor/${fileName}`;
...
function main() {
  const total = Object.keys(ARMORS).length;
  const images = findImagesById();

  console.log(`📦 Каталог:  ${total} позиций брони`);
  console.log(`🖼  Найдено:  ${images.size} картинок в icons/items/armor/`);

  if (images.size === 0) {
    console.log("Нечего применять. Сложи картинки в icons/items/armor/{id}.webp и запусти снова.");
    return;
  }
  ...
  console.log('  await game.ironHills.syncArmorPackFromCatalog()');
}
